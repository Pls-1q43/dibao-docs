...
- `packages/plugin-sdk`：plugin manifest、capabilities、events、signing、package validation helpers。
- `packages/plugin-cli`：third-party plugin template、validate、pack、sign commands。
- `plugins/official`：Daily Brief や Webhook などの bundled official plugins。
...
- Plugin Stable/Beta APIs、trusted signing keys、iframe bridge、persistence paths は `0.2` 実装と同期して維持します。
