...
Dibao `0.2.x` 起会随官方 Docker 镜像携带部分官方插件，例如 [Webhook](webhook/)。官方插件不需要上传 `.dibao-plugin` 文件，也不能像第三方插件一样卸载；打开设置页的 `插件` Tab，找到官方插件并启用即可。

本页后续步骤只适用于第三方插件。Webhook 等官方插件的配置方式请查看对应插件文档。
